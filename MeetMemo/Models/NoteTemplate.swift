        ...
        return [
            ...
            NoteTemplate(
                ...
                **沟通主题**：[用一句话概括本次沟通的核心议题]
                ...
                只有能根据发言人标签稳定区分双方时才分别归纳观点；无法区分时改为不归名的“主要观点”，不推测身份。
            ...
            NoteTemplate(
                ...
                **访谈目标**：[本次访谈希望了解的核心问题]
                ...
                优先级只在客户明确表达或有直接证据时填写；否则标注“未明确”，不按记录者的主观判断排序。

                ## 客户期望
                客户明确表达希望看到的解决方案、功能或结果。

                ## 隐性需求与洞察
                客户未明确说出，但有具体言语依据支持的深层诉求或顾虑。每条必须标注“分析判断”并简述依据；没有足够证据时省略本节。

                ## 关键引用语句
                仅当转录能够核对客户的准确表述时，摘录 1～3 句对理解需求最有价值的原话。转录质量或发言人归属不可靠时，改为“关键表述（转述）”或省略本节。
            ...
            NoteTemplate(
                ...
                **期望需求**
                会议中已提及、但尚未被确认为核心范围的需求。不得仅根据常识或背景材料自行添加需求。
            ...
            NoteTemplate(
                ...
                **面试官**：
                ...
                每项评价都必须先列出来自候选人回答的具体表现依据。信息不足的维度标注“信息不足，暂不判断”，不得根据语气、身份或背景臆测。
                ...
                推进建议属于分析判断，必须由前述具体证据支持；无足够证据时写“信息不足，暂不判断”。
                """,
                isDefault: true
            ),
