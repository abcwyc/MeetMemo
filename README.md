...
它适合日常会议、站会、1on1、客户访谈、需求评审、招聘面试等场景。所有会议数据保存在本机，语音识别使用 macOS 内置 SpeechAnalyzer 在本地运行，纪要生成的大模型服务由用户自行配置。
...
![](https://file.348580.xyz/2026/05/8a3ee308d1a77f9879f85e0eae6a29d5.png)
- 自定义会议总结提示词
...
- **音频文件导入**：支持导入音频或视频文件，并使用 macOS 本地语音识别生成转录。
...
- **本地语音识别**：转录使用 macOS 26 SpeechAnalyzer，无需 STT API Key；首次使用可能需要下载系统语音模型。
- **可配置 LLM 服务**：纪要生成支持 Anthropic Messages API，以及 OpenAI 兼容的 Chat Completions API。
...
## 项目说明
MeetMemo 基于开源项目 meetingnotes 二次开发，针对国内网络环境与实际业务场景做了以下改动：

- 本地语音识别：使用 macOS 26 SpeechAnalyzer 完成实时转录，减少外部 STT 服务依赖；LLM 端支持配置任意兼容 OpenAI 接口的模型（含 Anthropic），模型自选。
- 说话人识别（Speaker Diarization）：双路录音基础上支持区分发言人，纪要直接标注"谁说了什么"。
- 会前上下文注入：单场会议可预加载项目背景、专有名词或参考文档，生成纪要时带入业务语境，减少后期人工校对。
...
- **麦克风权限**：用于转录你在会议中说的话。
- **语音识别权限**：用于调用 macOS 本地 SpeechAnalyzer 生成转录。
...
在引导页或「设置 > 模型」中授权语音识别，并检查/安装 macOS 语音识别模型。转录在本地运行，无需 STT API Key；首次安装模型时可能需要网络。
...
- LLM 的 API Key、Token、Base URL、模型名保存在 macOS Keychain。
- MeetMemo 本身不提供云端账号系统，也不会把会议数据同步到项目自有服务器。
- 语音识别通过 macOS SpeechAnalyzer 在本机处理；纪要生成会把必要文本发送到你配置的 LLM 服务，请根据你的组织合规要求选择服务商。
...
2. 创建新 release，tag 使用 `v版本号`，例如 `v0.1`。
