...
- **会议列表管理**：支持搜索、重命名、删除会议，并可一键复制会前资料、转录原文或智能纪要；支持导出 HTML 或 Markdown 纪要。
...
- 打开任意会议后，可以复制当前标签页内容（会前资料、转录原文、智能纪要），或导出 HTML 纪要；也可以将 AI 纪要保存为 UTF-8 编码的 Markdown 文件，文件包含会议元数据并保留原始 Markdown 格式。
