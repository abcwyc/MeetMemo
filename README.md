...
- **本地语音识别**：默认使用本地 SenseVoice（sherpa-onnx）完成实时转录，减少对外部 STT 服务的依赖；macOS 26+ 用户也可切换到系统 SpeechAnalyzer。LLM 端支持配置任意 OpenAI 兼容接口的模型（含 Anthropic），模型自选。
...
- **本地 SenseVoice**：点击下载本地模型；推荐作为默认引擎，支持说话人识别。
