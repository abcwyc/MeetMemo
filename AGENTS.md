# AGENTS.md

This file provides guidance to Codex (Codex.ai/code) when working with code in this repository.

## Project Overview

MeetMemo (Meetingnotes) is a free, open-source macOS AI meeting notetaker. It captures mic + system audio, streams to the configured STT provider for live transcription, and generates structured notes via the configured LLM provider.

Two sub-projects:
- `meetingnotes/` — Native Swift/SwiftUI macOS app (primary)
- `landing-page/` — Next.js 15 marketing site

## Commands

### Native App (Xcode)

Build from command line (do **not** run the app after building):
```bash
xcodebuild -project Meetingnotes.xcodeproj -scheme meetingnotes -configuration Debug build
```

Release build (requires `.env` with `DEVELOPER_ID`, `APPLE_ID`, `TEAM_ID`, `APP_PASSWORD`):
```bash
./scripts/build_release.sh
```

Version management:
```bash
./scripts/update_version.sh [major|minor|patch|build|custom]
./scripts/verify_codesigning.sh
```

### Landing Page

```bash
cd landing-page
npm run dev    # Dev server
npm run build  # Production build
```

## Architecture

### Native App — MVVM Layers

**Models** (`meetingnotes/Models/`)
- `Meeting.swift` — Core data model: transcript chunks + generated notes
- `NoteTemplate.swift` — 7 built-in templates + custom templates
- `Settings.swift` — Provider credentials, prompts, onboarding state

**Managers** (singletons, `meetingnotes/Managers/`)
- `AudioManager.swift` — Opens dual WebSocket connections through `STTProviderFactory`; the current implementation uses Doubao STT for mic + system audio
- `RecordingSessionManager.swift` — Coordinates active recording state across the app
- `LocalStorageManager.swift` — JSON persistence to `~/Documents/Meetings/` and `~/Documents/Templates/`; uses atomic writes (temp file → replace)
- `KeychainHelper.swift` — Secure STT and LLM credential storage in macOS Keychain
- `DataMigrationManager.swift` — Versioned migration support for JSON data

**Services** (`meetingnotes/Services/`)
- `NotesGenerator.swift` — Builds template-aware prompts and streams note content through the configured `LLMProvider`
- `LLMClient.swift` — Routes Anthropic URLs to the Messages API and all other URLs to OpenAI-compatible chat completions
- `APIKeyValidator.swift` — Validates STT and LLM configs before allowing recording
- `ErrorHandler.swift` — Centralized error handling

**ProcessTap** (`meetingnotes/ProcessTap/`)
Low-level Core Audio subsystem for tapping system/app audio without UI access. `AudioProcessController` monitors running audio apps; `ProcessTap` manages tap lifecycle.

**ViewModels** (`meetingnotes/ViewModels/`)
SwiftUI `@Published` observable objects: `MeetingListViewModel`, `MeetingViewModel`, `SettingsViewModel`, `TemplatesViewModel`.

**Views** (`meetingnotes/Views/`)
SwiftUI views wired to ViewModels. Entry point is `ContentView.swift`; app bootstrap in `MeetingnotesApp.swift` (Sparkle + PostHog initialization).

### Key Data Flow

1. **Recording start** → `RecordingSessionManager` → `AudioManager` opens two WebSocket streams via the STT provider factory (one mic, one system audio; currently Doubao) → transcript chunks arrive and are appended to the active `Meeting` with a 2-second debounce before saving
2. **Note generation** → `NotesGenerator` reads transcript + selected `NoteTemplate` → `LLMClient` selects Anthropic Messages or OpenAI-compatible chat completions from the configured base URL → streams markdown notes back to `RenderedNotesView`
3. **Persistence** → all meetings/templates stored as JSON files; provider credentials in Keychain

### Landing Page

Next.js App Router under `landing-page/app/`. Pages: home, `/download`, `/privacy`, `/terms`. UI built with Shadcn (Radix UI + Tailwind CSS). No backend — purely static marketing pages.

## Key Configuration

| File | Purpose |
|------|---------|
| `meetingnotes/meetingnotes.entitlements` | Sandbox permissions: audio input, screen capture, network, mach lookups |
| `meetingnotes/Info.plist` | Sparkle update feed URL + EdDSA public key, audio usage descriptions |
| `.env.template` | Required env vars for code signing and notarization |
| `appcast.xml` | Sparkle auto-update feed (generated by release script) |

## Important Constraints

- **Build without running**: When modifying the native app, build to verify compilation but do not launch the app.
- **Code signing**: The release binary requires a Developer ID Application certificate. Debug builds run unsigned locally.
- **Universal binary**: Release builds target both `arm64` and `x86_64`.
- **Conventional commits**: Follow conventional commit format (`feat:`, `fix:`, `chore:`, etc.) per `CONTRIBUTING.md`.
- **Contributor License Agreement**: All contributions grant the project rights to relicense under LGPL-3.0.
